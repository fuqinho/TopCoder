...
public:
  long long countSubsets(int nlow, int nhigh, vector <int> px, vector <int> py) {
      ...
      for (int right = left; right < x.size()-1; right++) {
        for (int top = 1; top < y.size()-1; top++) {
          for (int bottom = top; bottom < y.size()-1; bottom++) {
            ...
            if (width <= nhigh && height <= nhigh && outer_width > nlow && outer_height > nlow && max(width, height) < min(outer_width, outer_height)) {
              LL mask = 0;
...
namespace moj_harness {
	...
	int run_test_case(int casenum) {
		...
		case 0: {
			...
			long long received__      = RangeSquaredSubsets().countSubsets(nlow, nhigh, vector <int>(x, x + (sizeof x / sizeof x[0])), vector <int>(y, y + (sizeof y / sizeof y[0])));
